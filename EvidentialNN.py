...
# =============== 模型保存路径 ===============
save_path = r"C:\Users\WHQ\Desktop\evidentialNN"
savepath_csv = r"C:\Users\WHQ\Desktop\evidentialNN\la23-24-24\la23-24-24.csv"
# 存储四种结构
save_str_dir = r"C:\Users\WHQ\Desktop\evidentialNN\la23-24-24"
third_dataset_path_db = os.path.join(save_path, "la23-24-24.db")
...
custom_data = spk.data.AtomsDataModule(
    r"C:\Users\WHQ\Desktop\evidentialNN\la23-24.db",
    batch_size=10,
    distance_unit='Ang',
...
if __name__ == '__main__':
    ...
    pred_U0 = spk.atomistic.EvidentialNN(n_in=n_atom_basis, output_key="energy_U0")
    ...
    evidential_loss = spk.task.EvidentialLoss(reg_coeff=0.1)

    output_U0 = spk.task.ModelOutput_ENN(
        name="energy_U0",
        loss_fn=evidential_loss,
        loss_weight=1.,
    ...
    logger = pl.loggers.CSVLogger(
        save_dir=r"C:\Users\WHQ\Desktop\evidentialNN\la23-24-24",
        name='lasi23-24-24'
    )
    ...
    for batch in predict_data.train_dataloader():
        ...
        # 获取ENN预测结果（关闭梯度计算，保留你的原始逻辑）
        with torch.no_grad():
            outputs = model(batch)

        # 提取预测能量（保留你的原始键名 "energy_U0"，兼容模型输出）
        energies = outputs["energy_U0"].cpu().detach().numpy()
        all_energies.append(energies)

        # =============== 核心修改1：适配证据神经网络的不确定性提取 ===============
        uncertainties = None  # 初始化不确定性

        # 获取ENN的不确定性缓存（包含认知不确定性/偶然不确定性/总不确定性）
        enn_uncertainty_dict = model.output_modules.get_cached_uncertainty()
        # 优先选择总不确定性（最常用，也可根据需求选择 "epistemic"（认知）或 "aleatoric"（偶然））
        uncertainties = enn_uncertainty_dict["total"].cpu().detach().numpy()
    ...
    model.output_modules.export_tsne_csv(r"C:\Users\WHQ\Desktop\evidentialNN\lasi23-24\lasi23-24_tsne.csv")
    ...
    #
    # # 可选：打印统计信息
    # print(f"能量最低: {all_energies[idx_lowest_energy].item():.6f} (不确定性: {all_uncertainties[idx_lowest_energy].item():.6f})")
    # print(f"能量最高: {all_energies[idx_highest_energy].item():.6f} (不确定性: {all_uncertainties[idx_highest_energy].item():.6f})")
    # print(
    ...
    #
    # # 清空旧数据库（如果存在）
    # if os.path.exists(third_dataset_path_db):
    #     os.remove(third_dataset_path_db)
    #
    # # 保存选中的结构到数据库
    ...
    #     third_dataset_path_db,
    #     distance_unit="Ang",
    #     property_unit_dict={"energy_U0": "eV"},
    # )
    # new_dataset.add_systems(property_list, atoms_list)
    # print(f"已将 {len(diverse_indices)} 个结构保存到 {third_dataset_path_db}")
    #
    # # =============== 结束计时 ===============
    # end_time = time.time()
