...
# Paths
# =========================
save_path = f"{output_dir}/mc_dropout"
save_str_dir = f"{output_dir}/{cfg['files']['structure_dir']}"
third_dataset_path = f"{output_dir}/{cfg['files']['db_name']}"
csv_path = f"{output_dir}/{cfg['files']['csv_name']}"
...
train_data = spk.data.AtomsDataModule(
    ...
    num_train=0.9,
    num_val=0.1,
    split_file=cfg["data"]["split_file_train"],
    transforms=[
...
predict_data = spk.data.AtomsDataModule(
    ...
    num_train=0.9,
    num_val=0.1,
    split_file=cfg["data"]["split_file_pred"],
    transforms=[
        trn.ASENeighborList(cutoff=cutoff),
        trn.RemoveOffsets("energy_U0", remove_mean=True),
        trn.CastTo32(),
...
trainer = pl.Trainer(
    ...
    logger=pl.loggers.CSVLogger(output_dir, name="mc_run"),
)
